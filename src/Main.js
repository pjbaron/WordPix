...
const Tightness = 10;		// how hard it will try to fit words cleverly
const Padding = 4;			// space around each word
	...
	switch(this.state)
	{
		case States.LoadImage:
			...
			if (newState)
			{
				...
				this.image.src = "./data/image_erode.png";
			}
			break;
...
Main.prototype.fitSize = function( _sizeIndex )
{
	...
	for(var i = 0, l = Math.ceil(this.indices.length * (1 - WordSpread)); i < l; i++)
	{
		...
		var w = size.w;
		var h = size.h;
		var tries = Tightness * _sizeIndex;
